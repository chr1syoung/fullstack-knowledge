...
## 1.2.23 AI 聊天气泡布局（自动撑开 + 最大高度滚动）

#### 知识点详解

**核心需求分析：**
- 用户消息气泡：宽度受限，最大高度内自适应撑开，超出最大高度滚动
- AI 消息气泡：同用户气泡，但通常 AI 气泡更长，需要特殊处理
...
.chat-container {
  ...
  flex-direction: column;
  height: 100vh;
  height: 100dvh; /* 动态视口高度，兼容 iOS 底部栏 */
}
...
document.documentElement.style.setProperty('--bubble-max-height', maxH + 'px');
```
...
const ChatContainer = () => {
  ...
  return (
    <div className="chat-container">
      <div className="message-area" ref={listRef}>
        {messages.map((m) => (
...
**题目：在实现 AI 聊天对话框时，如何使用 CSS 实现一个自动撑开且支持最大高度滚动的气泡布局？**
...
.bubble {
  max-width: 70%;
  max-height: 400px;          /* 超出这个高度就滚动 */
  overflow-y: auto;            /* 自动出现滚动条 */
  height: auto;               /* 内容少时自动收缩 */
  word-break: break-word;      /* 长单词换行，防止溢出 */
}
...
.message-list {
  ...
  gap: 12px;
  flex: 1;
  overflow-y: auto;           /* 整个消息区滚动 */
}

/* 关键：flex 子项设置 min-height: 0 */
.message-list > * {
  min-height: 0;
}
```
...
**面试加分点：**
- 提到 `height: auto` + `max-height` 的组合实现自适应 + 上限
- 提到 `flex: 1` + `min-height: 0` 防止 flex 子项撑开父容器
- 提到 `height: 100dvh` 适配 iOS Safari 动态视口
- 提到 `env(safe-area-inset-bottom)` 适配 iPhone 底部手势区域
---
---
